...
*/
	function get_reservas_bydate($fecha,$scope){
			$scp = ($scope === 'all')?'>=':"="; 
			$q= "SELECT r.fecha_reserva fecha, s.id paseo, horas.id hora, r.cant_pasajeros_reserva pax ,r.monto_pagado_reserva senia ,r.monto_total_reserva saldo,s.tarifa tarifa,u.usr_usuario operador ,cl.nombre_contacto_cliente cliente,cl.email_contacto_cliente email,cl.telefono_contacto_cliente tel ,r.observaciones_reserva detalle  FROM cat_reservas r LEFT OUTER JOIN usuarios u ON r.usuarios_id = u.id LEFT OUTER JOIN cat_historial_servicios hs ON r.historial_servicios_id = hs.id LEFT OUTER JOIN cat_horarios horas on horas.id = hs.horarios_id LEFT OUTER JOIN cat_clientes cl ON r.clientes_id = cl.id LEFT OUTER JOIN cat_barcos b ON b.id = hs.barcos_id LEFT OUTER JOIN cat_servicios s ON hs.servicios_id = s.id WHERE r.fecha_reserva {$scp} '{$fecha}' ORDER BY r.fecha_reserva, hs.horarios_id ASC";
			$x = $this->db->query($q);
			return $x -> result_array();
	}
